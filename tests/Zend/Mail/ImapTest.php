...
class Zend_Mail_ImapTest extends PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        if (defined('TESTS_ZEND_MAIL_SERVER_TESTDIR') && TESTS_ZEND_MAIL_SERVER_TESTDIR) {
            ...
            $this->_copyDir(TEST_ROOT_DIR . '/Zend/Mail/_files/test.' . TESTS_ZEND_MAIL_SERVER_FORMAT,
    ...
    protected function _copyDir($dir, $dest)
    {
        ...
        while (($entry = readdir($dh)) !== false) {
            ...
            if (is_dir($fullname)) {
                $old = umask(0);
                mkdir($destname, 0777); // dovecot [NOPERM] Permission denied
                umask($old);
                $this->_copyDir($fullname, $destname);
            } else {
                copy($fullname, $destname);
                chmod($destname, 0666); // dovecot [NOPERM] Permission denied
            }
        }
        closedir($dh);
    }
...
*/
    ...
    public function testStore()
    {
        ...
        $this->assertEmpty($result); // flags not changed, $result = array()
        $result = $protocol->store(array('\Flagged'), 1, null, '-', false);
